							...
							u8 *param)
{
	...
	/*** add your codes here ***/
	f = filp_open("/data/miniISP_memory.log", O_APPEND|O_CREAT, 0777);

/*ZTEMT: fengxun modify for NULL pointer bug--------Start*/
	if(IS_ERR(f)){
		goto allocate_memory_fail;
	}
/*ZTEMT: fengxun modify for NULL pointer bug--------End*/
...
errcode bulk_data_cmd_read_common_log(struct misp_data *devdata, u8 *param)
{
	...
	f = filp_open("/data/miniISP.log", O_APPEND|O_CREAT, 0777);
