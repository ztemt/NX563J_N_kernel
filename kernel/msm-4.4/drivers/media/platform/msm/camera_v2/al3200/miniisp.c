...
static int vcc3_gpio = -1;

/*ZTEMT: fengxun add for poweron--------Start*/
static u8 is_power_on = 0;
/*ZTEMT: fengxun add for poweron--------End*/
...
/*ZTEMT: fengxun add for AL3200--------Start*/
	if(0 == is_power_on){
		misp_info("%s - enter but in power down status, return here", __func__);
		return IRQ_HANDLED;
	}
...
int mini_isp_spi_recv(struct misp_data *devdata, u32 len, bool wait_int)
{
	...
	*/
	status =  spi_write_then_read(devdata->spi, spi_databuffer,1,
			spi_databuffer+1, len);
	if (status) {
		misp_err("%s - sync error: status=%d", __func__, status);
		goto mini_isp_spi_recv_end;
	}
...
//			spi_bulkbuffer = kzalloc(SPI_BULK_SIZE,GFP_DMA);//ZTEMT:fengxun modify from vmalloc
//		/* Allocate boot code bulk buffer*/
//		else
//			spi_bulkbuffer = kzalloc(SPI_BULK_SIZE_BOOT,GFP_DMA);//ZTEMT:fengxun modify from vmalloc
...
T_EXIT:
	...
	if (spi_bulkbuffer != 0){
		//kfree(spi_bulkbuffer);//ZTEMT:fengxun modify from vfree
...
int mini_isp_get_chip_id(u32 mini_isp_reg_addr, u8 *id_buf)
{
    ...
    u8 recv_buffer_spi_value[64]={0};
	
	devdata = get_mini_isp_data();
	send_buffer_spi = send_buffer_spi_value;
	recv_buffer_spi = recv_buffer_spi_value;
	...
	status = mini_isp_spi_sync(devdata->spi, send_buffer_spi,
		send_buffer_spi, 63);
#else
    status = spi_write_then_read(devdata->spi,send_buffer_spi_value, 5, recv_buffer_spi_value , 26);
    memcpy(send_buffer_spi,recv_buffer_spi,26);
#endif
	if (status) {
		misp_err("%s - sync error: status=%d", __func__, status);
...
#if 0
	while (cont < 10) {
		pr_err("[miniISP]Get Chip ID[cont=%d] =%x ",cont,*(send_buffer_spi+cont));
		cont++;
	}
	cont = 0;
#endif
    ...
    */
	if (*send_buffer_spi == 0xa5){
		misp_info("[miniISP]duyuerong Get Chip ID %x %x %x %x",
			*(send_buffer_spi+1), *(send_buffer_spi+2),
			*(send_buffer_spi+3), *(send_buffer_spi+4));
...
void mini_isp_poweron(void)
{
    ...
    gpio_set_value(vcc3_gpio, 0);
    msleep(2);
    gpio_set_value(vcc1_gpio, 1);
    msleep(2);
    gpio_set_value(vcc2_gpio, 1);
    msleep(2);
    gpio_set_value(vcc3_gpio, 1);
    msleep(5);
    if (clk_prepare_enable(isp_clk) < 0)
    {
...
/*ZTEMT: fengxun add for poweron--------Start*/
    is_power_on = 1;
/*ZTEMT: fengxun add for poweron--------End*/
    misp_err("%s -reset_gpio gpio_get_value = %d", __func__,gpio_get_value(reset_gpio));
    misp_err("%s -vcc1_gpio gpio_get_value = %d", __func__,gpio_get_value(vcc1_gpio));
    misp_err("%s -vcc2_gpio gpio_get_value = %d", __func__,gpio_get_value(vcc2_gpio));
    misp_err("%s -vcc3_gpio gpio_get_value = %d", __func__,gpio_get_value(vcc3_gpio));
...
static int mini_isp_probe(struct spi_device *spi)
{
	...
	misp_info("chengjiatest: mini isp probe\n");

	/*step 1: alloc driver data struct*/
	spi_bulkbuffer = kmalloc(SPI_BULK_SIZE,GFP_DMA);//ZTEMT:fengxun modify for buf alloc
	if (!spi_bulkbuffer) {
		ret = -ENOMEM;
	...
	ret = misp_ts_pinctrl_init(spi, true);
        misp_info("%s - misp_ts_pinctrl_init done.", __func__);
	...
	vcc3_gpio = of_get_named_gpio(spi->dev.of_node, "vcc3-gpios", 0);
	misp_err("%s - probe vcc3-gpios = %d", __func__, vcc3_gpio);

	ret = devm_gpio_request(&spi->dev, vcc3_gpio, "vcc3-gpios");
	if (ret) {
		misp_err("%s -step 4. request vcc3-gpio error", __func__);
		goto err_gpio3_config;
	}

	gpio_direction_output(vcc3_gpio,  1);
	gpio_set_value(vcc3_gpio, 1);
	msleep(20);

      isp_clk = devm_clk_get(&spi->dev, "al3200_clk");
      misp_err("clk_ptr = %p", isp_clk);  
      ret = clk_set_rate(isp_clk, 19200000L);      
      if (ret < 0)
      {
...
err_spi_setup:
	kfree(drv_data);
	kfree(spi_bulkbuffer);//ZTEMT:fengxun modify for buf alloc
	spi_bulkbuffer = NULL;//ZTEMT:fengxun modify for buf alloc
	misp_drv_data = NULL;
mini_isp_probe_end:
	return ret;
}
...
#if 1
//========================================
//===== ID:22  cam0_res0 + cam1_res0
//===== MCK=19.20MHZ
//===== RX0=1296.00Mbps  4 Lane
//===== RX1=1218.00Mbps  4 Lane
//========================================
// SCID=22
...
udelay(350);
mini_isp_send_register_chang_buffer(0xffe81040, 0x00001806);
mini_isp_send_register_chang_buffer(0xffe81044, 0x00000c03);
mini_isp_send_register_chang_buffer(0xffe81048, 0x0000012f);
mini_isp_send_register_chang_buffer(0xffe81004, 0x00000000);
udelay(350);
mini_isp_send_register_chang_buffer(0xffe800e0, 0x0000080a);
mini_isp_send_register_chang_buffer(0xffe80100, 0x0000080a);
mini_isp_send_register_chang_buffer(0xffe80120, 0x0000080a);
mini_isp_send_register_chang_buffer(0xffe81004, 0x00000800);
udelay(350);
mini_isp_send_register_chang_buffer(0xffe81058, 0x00000198);
mini_isp_send_register_chang_buffer(0xffe8105c, 0x000000cc);
mini_isp_send_register_chang_buffer(0xffe81060, 0x00000102);
mini_isp_send_register_chang_buffer(0xffe81004, 0x00000000);
udelay(350);
mini_isp_send_register_chang_buffer(0xffe80b00, 0x0000043f);
mini_isp_send_register_chang_buffer(0xffe80880, 0x00000400);
mini_isp_send_register_chang_buffer(0xffe80940, 0x00000800);
mini_isp_send_register_chang_buffer(0xffe808c0, 0x00000400);
mini_isp_send_register_chang_buffer(0xffe80980, 0x00000800);
mini_isp_send_register_chang_buffer(0xffe80400, 0x00000402);
...
mini_isp_send_register_chang_buffer(0xffed1008, 0x00000a12);
mini_isp_send_register_chang_buffer(0xffed100c, 0x0000020a);
mini_isp_send_register_chang_buffer(0xffed1010, 0x00000029);
mini_isp_send_register_chang_buffer(0xffed1014, 0x00000f0a);
mini_isp_send_register_chang_buffer(0xffed1018, 0x00000a0b);
mini_isp_send_register_chang_buffer(0xffed101c, 0x00000a12);
...
mini_isp_send_register_chang_buffer(0xfff9700c, 0x13b00084);
...
mini_isp_send_register_chang_buffer(0xfff9106c, 0x00001f1f);
...
mini_isp_send_register_chang_buffer(0xffed6008, 0x00000a11);
mini_isp_send_register_chang_buffer(0xffed600c, 0x0000020a);
mini_isp_send_register_chang_buffer(0xffed6010, 0x00000026);
mini_isp_send_register_chang_buffer(0xffed6014, 0x00000e0a);
mini_isp_send_register_chang_buffer(0xffed6018, 0x0000090a);
mini_isp_send_register_chang_buffer(0xffed601c, 0x00000911);
...
mini_isp_send_register_chang_buffer(0xfff9800c, 0x13b0008c);
mini_isp_send_register_chang_buffer(0xfff98010, 0x00000000);
mini_isp_send_register_chang_buffer(0xfff98024, 0x00000028);
mini_isp_send_register_chang_buffer(0xfff98028, 0x00000000);
mini_isp_send_register_chang_buffer(0xfff9802c, 0x0000000c);
mini_isp_send_register_chang_buffer(0xfff98030, 0x12121212);
...
mini_isp_send_register_chang_buffer(0xfff94028, 0x00003737);
...
mini_isp_send_register_chang_buffer(0xfff9406c, 0x00001d1d);
